...
app.get('/friends', authenticateToken, async (req, res) => {
    ...
    try {
        const query = `
            ...
            FROM users u
            INNER JOIN friend_requests fr ON u.id = fr.to_user_id
            WHERE fr.from_user_id = ? AND fr.status = 'aceita'
            UNION   
            SELECT u.id, u.username
            FROM users u
            INNER JOIN friend_requests fr ON u.id = fr.from_user_id
            WHERE fr.to_user_id = ? AND fr.status = 'aceita';
        `;
        console.log('[BACKEND] /friends: Executando consulta para amigos do usuário ID: ${currentUserId}.');
        const [friends] = await pool.query(query, [currentUserId, currentUserId]);

        console.log('[BACKEND] /friends: Encontrados ${friends.length} amigos para o usuário ID: ${currentUserId}');
        res.json(friends);
    } catch (error) {
        console.error(`[BACKEND] /friends: Erro ao buscar lista de amigos para o usuário ID ${currentUserId}:`, error.message);
        if (error.sqlMessage) {
             console.error(`[BACKEND] /friends: SQL Error: ${error.sqlMessage}`);
        }
        res.status(500).json({ message: 'Erro ao buscar lista de amigos.' });
    }
});
